...
export default function BuatKelasPage() {
  ...
  // Enrollment settings (global untuk semua siswa yang dipilih)
  const [sessionsTotal,       setSessionsTotal]       = useState(8)
  const [sessionStartOffset,  setSessionStartOffset]  = useState(1)
    ...
    <div className="max-w-xl">
      ...
      <div className="bg-white rounded-2xl border border-[#E5E3FF] p-6">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* Pengaturan Paket — tampil hanya jika ada siswa dipilih */}
          {selectedStudents.length > 0 && (
            <div className="bg-[#F7F6FF] border border-[#E5E3FF] rounded-xl p-4 space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-[#7B78A8] uppercase tracking-wide mb-1.5">
                  ...
                  <span className="normal-case font-normal ml-1 text-[#7B78A8]">(default 8)</span>
                </label>
              ...
              <div>
                <label className="block text-xs font-bold text-[#7B78A8] uppercase tracking-wide mb-1.5">
                  Mulai dari Sesi ke-
                  <span className="normal-case font-normal ml-1 text-[#7B78A8]">(default 1)</span>
                </label>
                <input type="number" min={1} max={sessionsTotal} value={sessionStartOffset}
                  onChange={e => setSessionStartOffset(Number(e.target.value))}
                  className={inputClass} />
                <p className="text-xs text-[#7B78A8] mt-1">
                  {sessionStartOffset === 1
                    ? 'Paket baru mulai dari awal.'
                    : `Sesi pertama di sistem akan tampil sebagai sesi ke-${sessionStartOffset} dari ${sessionsTotal}.`}
                </p>
              </div>
            </div>
          )}
