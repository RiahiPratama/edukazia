...
import { X, ArrowRightLeft, UserMinus, Check } from 'lucide-react'
...
export default function EditKelasPage() {
  ...
  // Modal state
  const [modalType,        setModalType]        = useState<'transfer' | 'stop' | null>(null)
  ...
  async function fetchAll() {
    ...
    if (k) {
      setKelas(k as any)
      setFLabel((k as any).label)
      setFZoom((k as any).zoom_link ?? '')
      setFStatus((k as any).status)
    }

    // Fetch enrollments + student names (two step)
    ...
    // Fetch kelas lain (untuk transfer)
  ...
  async function handleTransfer() {
    ...
    // Hitung sisa sesi
    const sisaSesi = selectedEnroll.sessions_total - (selectedEnroll.sessions_used ?? 0)
    ...
    // Buat enrollment baru di kelas tujuan
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-xs text-[#7B78A8]">
                      Sesi {enr.session_start_offset}/{enr.sessions_total}
                    </span>
                    {enr.notes && (
      ...
      {modalType === 'stop' && selectedEnroll && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
            ...
            <p className="text-sm text-[#7B78A8] mb-2">
              <strong>{selectedEnroll.student_name}</strong> akan ditandai berhenti dari kelas ini.
            </p>
