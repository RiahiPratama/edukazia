      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {loading ? '...' : `${filtered.length} siswa terdaftar`}
          </p>
        </div>
        <Link
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ backgroundColor: '#F7F6FF' }}>
              ...
              <th className="text-left px-5 py-3.5 font-semibold text-gray-500 text-xs uppercase tracking-wider">Nama Siswa</th>
              <th className="text-left px-5 py-3.5 font-semibold text-gray-500 text-xs uppercase tracking-wider hidden md:table-cell">Email</th>
              <th className="text-left px-5 py-3.5 font-semibold text-gray-500 text-xs uppercase tracking-wider hidden lg:table-cell">Telepon</th>
              <th className="text-left px-5 py-3.5 font-semibold text-gray-500 text-xs uppercase tracking-wider hidden xl:table-cell">Terdaftar</th>
              <th className="text-right px-5 py-3.5 font-semibold text-gray-500 text-xs uppercase tracking-wider">Aksi</th>
            </tr>
          </thead>
            ...
            {loading ? (
                ...
                <td colSpan={6} className="px-5 py-16 text-center">
                  <div className="flex flex-col items-center gap-3 text-gray-400">
                    ...
                    <p className="font-medium text-gray-500">
                      {search ? 'Siswa tidak ditemukan' : 'Belum ada siswa terdaftar'}
                    </p>
                    {!search && (
                      <Link
                        href="/admin/siswa/baru"
              ...
              paginated.map((siswa, idx) => {
                ...
                return (
                  <tr
                    key={siswa.id}
                    className="bg-white hover:bg-[#F7F6FF] transition-colors"
                  >
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          ...
                          className="p-1.5 rounded-lg text-gray-400 hover:text-[#5C4FE5] hover:bg-[#F0EEFF] transition-colors"
                          title="Edit"
                        >
                          <Pencil size={15} />
                        </Link>
          ...
          >
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                disabled={page === 1}
                className="p-1.5 rounded-lg disabled:opacity-40 hover:bg-white transition-colors border"
                ...
                .map((p, i) =>
                  p === '...' ? (
                    ...
                    <button
                      ...
                      style={{
                        backgroundColor: page === p ? '#5C4FE5' : 'white',
                        color: page === p ? 'white' : '#374151',
                        border: `1px solid ${page === p ? '#5C4FE5' : '#E5E3FF'}`,
                      }}
                    >
                      {p}
                    </button>
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 shadow-xl max-w-sm w-full">
            ...
            <h3 className="text-lg font-bold text-gray-800 mb-1">Hapus Siswa?</h3>
            <p className="text-sm text-gray-500 mb-6">
              Data siswa akan dihapus permanen dan tidak dapat dikembalikan.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setDeleteId(null)}
                className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-gray-600 border transition-colors hover:bg-gray-50"
                style={{ borderColor: '#E5E3FF' }}
              >
                Batal
              </button>
