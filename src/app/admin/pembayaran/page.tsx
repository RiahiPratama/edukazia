...
export default function PendapatanPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    // 1. Fetch payments — kolom sesuai skema real
    ...
    if (rawPayments && rawPayments.length > 0) {
      // 2. payments → enrollment_id → class_group_id
      ...
      // 3. enrollments → class_group_id → course_id + class_type_id
    ...
    const { data: rawSubs } = await supabase
      ...
      .select(
        'id, plan_name, price_paid, payment_method, status, start_date, end_date, created_at, tutor_id, course_id'
      )
  ...
  // Reset filter status saat ganti tab
  ...
  // ── Filter ────────────────────────────────────────────────────────────────
  // Filter bulan dari paid_at (bukan period_label — formatnya tidak konsisten)
  ...
  // ── Summary — ikut filter bulan yang aktif ────────────────────────────────
  ...
  // Daftar bulan dari paid_at — bukan period_label yang formatnya tidak konsisten
  const allMonths = [
    ...new Set([
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Pembayaran kursus siswa + subscription B2B
          </p>
        </div>
        <button
          onClick={fetchData}
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          },
          {
            ...
            sub: `${filteredPayments.filter((p) => p.status === 'pending').length} transaksi pending`,
            icon: <Clock size={20} />,
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <div className="flex rounded-xl border border-[#E5E3FF] bg-white p-1 shadow-sm">
          {(['kursus', 'b2b'] as const).map((tab) => (
            ...
            >
              {tab === 'kursus' ? 'Kursus Siswa' : 'Subscription B2B'}
            </button>
          ))}
        </div>

        {/* Filter Bulan — dari paid_at bukan period_label */}
