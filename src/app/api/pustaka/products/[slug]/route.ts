...
export async function GET(
  ...
  try {
    ...
    // Query 3: slide (metadata saja, tanpa image_url & hotspots — butuh akses untuk lihat itu)
    ...
    // Cek akses user (kalau login)
    const { data: { user } } = await supabase.auth.getUser()
    let accessInfo = { has_access: false, access_type: 'none', expires_at: null as string | null }
