// src/app/api/materials/share/route.ts

import { createClient } from '@supabase/supabase-js'
import { NextRequest, NextResponse } from 'next/server'
import { extractFileId, batchShareFiles } from '@/lib/googleDrive'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!materials || materials.length === 0) {
      return NextResponse.json(
        { message: 'No materials found', shared: 0 },
        { status: 200 }
      )
    }

    // Extract Google Drive File IDs from content_data
    ...
    if (fileIds.length === 0) {
      return NextResponse.json(
        { message: 'No valid Google Drive URLs found', shared: 0 },
        { status: 200 }
      )
    }

    // Share all files to student email
    const result = await batchShareFiles(fileIds, studentEmail)
    ...
    await supabase.from('material_access_logs').insert({
      ...
      metadata: {
        level_id: levelId,
        email: studentEmail,
        files_shared: result.successful,
    ...
    console.error('Share materials error:', error)
    return NextResponse.json(
      { error: error.message },
      { status: 500 }
    )
